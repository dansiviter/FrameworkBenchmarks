...
public class JdbcRepository implements DbRepository {
    ...
    public Single<World> getWorld(int id) {
		return this.client.execute(exec -> {
			LOGGER.finest(() -> "getWorld(...)");
			return exec.namedGet("select-world-by-id", id);
    ...
    public Single<World> updateWorld(World world) {
		return this.client.execute(exec -> {
			LOGGER.finest(() -> "updateWorld(...)");
			return exec.namedUpdate("update-world-by-id", world.randomNumber, world.id);
		}).map(r -> world);
    }
